...
class Units(views.APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            max_price = int(self.request.query_params.get('max_price', None))

            # Checking reservations from requested date range
            reservations_ids = Reservation.objects.filter(check_in__range=(check_in, check_out)).values('id')                                                          \
                           &   \
                           Reservation.objects.filter(check_out__range=(check_in, check_out)).values('id')
            ...
            # max_price filter
            if max_price:
                booking_info = booking_info.filter(price__lte=max_price)
